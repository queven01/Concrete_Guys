...
//Tickets Active Button
function _activeButton(){
var current = location.pathname.replace(/\/$/, '');
var curretSeg = current.substr(current.lastIndexOf('/') + 1);

$('.content-section.tickets a.btn').each(function(){
    var $this = $(this);
    var url= $this.attr('href').replace(/\/$/, '');
    var lastSeg = url.substr(url.lastIndexOf('/') + 1);
...
//         const $container = document.querySelector('.site'); // Target container
...
//             y: y + 3,
//             duration: 1,
//             ease: 'back', // More easing options here: https://gsap.com/docs/v3/Eases/
//             opacity: isTargetLinkOrBtn ? 0.5 : .8,
//             transform: `scale(${isTargetLinkOrBtn ? 0 : 1})`,
//             // width: isTargetLinkOrBtn ? '10px' :  '40px;',
//             // height: isTargetLinkOrBtn ? '10px' :  '40px;',
//             // left: isTargetLinkOrBtn ? '-1rem' :  '',
//             // top: isTargetLinkOrBtn ? '1.3rem' :  '',
//           });
//         });
